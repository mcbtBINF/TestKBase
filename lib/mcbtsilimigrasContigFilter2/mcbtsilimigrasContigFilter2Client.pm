package mcbtsilimigrasContigFilter2::mcbtsilimigrasContigFilter2Client;
...
mcbtsilimigrasContigFilter2::mcbtsilimigrasContigFilter2Client
...
sub new
{
    ...
    my $self = {
	client => mcbtsilimigrasContigFilter2::mcbtsilimigrasContigFilter2Client::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a mcbtsilimigrasContigFilter2.FilterContigsParams
$output is a mcbtsilimigrasContigFilter2.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a mcbtsilimigrasContigFilter2.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a mcbtsilimigrasContigFilter2.assembly_ref
...
# Authentication: required
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "mcbtsilimigrasContigFilter2.filter_contigs",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "mcbtsilimigrasContigFilter2.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub version {
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "mcbtsilimigrasContigFilter2.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for mcbtsilimigrasContigFilter2::mcbtsilimigrasContigFilter2Client\n";
    }
    if ($sMajor == 0) {
        warn "mcbtsilimigrasContigFilter2::mcbtsilimigrasContigFilter2Client version is $svr_version. API subject to change.\n";
    }
}
...
package mcbtsilimigrasContigFilter2::mcbtsilimigrasContigFilter2Client::RpcClient;
